...
**Caveman ultra enforced** on edits to `AGENTS.md`, `DESIGN.md*`, `SPEC.md`,
...
`@explore` (Task tool, subagent `explore`). Still no answer → grep/Read last. No
`.codegraph/` dir or no index? → `@explore` first. Indexing user choice.
