...
- **No `any`** without a comment justifying why (and those cases should be
  rare). Use `unknown` and type narrowing instead.
...
- JSON-LD structured data in `<script type="application/ld+json">` within layout
  or page components.
...
- **`main` (Production):** Exclusively contains 100% stable code. Every update
  ...
  repository. It only receives changes via merges from `release/*` or `hotfix/*`
  branches.
...
- **Frontend Design (`frontend-design`):** BOLD, anti-generic aesthetics.
  ...
  high-impact motion choreography. No Inter/Roboto/Arial. No purple gradients on
  white.
...
- **Accessibility (`accessibility`):** WCAG 2.2 Level AA compliance — POUR
  principles, semantic HTML/JSX, keyboard navigation, ARIA, color contrast, form
  validation.
